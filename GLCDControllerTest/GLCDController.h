...
#include "CanvasWithUpdateFlag.h"
#include "Arduino.h"
    ...
    for (byte i = 0; i < 2; i++)
    {
      ...
      for (int page = 0; page < 8; page++)
      {
        ...
        for (int address = 0; address < 64; address++)
        {
          ...
          {
            // 前のアドレスが前回更新されたものでないとき
            if (address - 1 != updatedAddressPrev)
            {
              // 前のアドレスが前回更新されたアドレスと等しい場合は,
              // 改めてアドレス指定を行う必要はない. こうすることで, 描画速度の向上が期待できる.
              // このグラフィック液晶は, 連続してWriteDataを行うと自動でアドレスがインクリメントされる.
